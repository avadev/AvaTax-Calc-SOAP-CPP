...
void GetTaxTest::Test()
{
	...
	try
	{
		...
		line2->No = "0002";
		line2->ItemCode = "T345";		
		line2->Qty = Convert::ToDouble(3);
		line2->Amount = Convert::ToDecimal(150);
		...
		Console::WriteLine("GetTaxTest Result: {0}", 
			getTaxResult->ResultCode.ToString());
		if(getTaxResult->ResultCode != SeverityLevel::Success)
		{
			for each (Message^ message in getTaxResult->Messages )
			{
				Console::WriteLine("{0}\n{1}\n{2}\n{3}", 
					message->Name,
					message->Summary,
		...
		else
		{
			Console::WriteLine("Document Code: {0}\nTotal Tax: {1}", 
				...
				getTaxResult->TotalTax.ToString());
			
			for each(TaxLine^ taxLine in getTaxResult->TaxLines)
			{	
